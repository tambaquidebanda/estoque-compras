-- RELATORIO: produtos que ainda estao SEM Pedido Padrao, por setor/grupo.
-- Cruza estrutura + adicoes (menos os excluidos) contra a chave 'padroes'.
-- Read-only, UMA unica instrucao. As CASE WHEN ...='object'/'array' evitam erro
-- caso algum no do jsonb esteja null (como ja aconteceu em adicoes).
...
faltando AS (
  SELECT t.setor, t.grupo, t.produto
  FROM todos t
  LEFT JOIN padr pd
...
SELECT setor,
       grupo,
       count(*)                                    AS faltando,
       string_agg(produto, ', ' ORDER BY produto)  AS produtos_sem_padrao
FROM faltando
GROUP BY setor, grupo
ORDER BY setor, grupo;

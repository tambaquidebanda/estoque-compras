...
const INVENTARIO_ESTRUTURA = {
  ...
  "BAR": {
    ...
    "ALCOOLICAS": ["MP BARRIL CHOPP BRAHMA 50 LITROS","MP BUDWEISER LN","MP CORONA LN","MP ORIGINAL 600ML","MP SPATEN 600ML","MP STELLA ARTOIS 600ML","MP CORONA ZERO LN","MP ORIGINAL LN","MP MICHELOB LN","MP SPATEN LN","MP STELLA ARTOIS LN","MP STELLA ARTOIS PURE GOLD LN","MP BUDWEISER ZERO LN","MP BOHEMIA 600ML"],
    "DESTILADOS": ["MP CACHAÇA 51 960ML","MP CACHAÇA LEBLON 750ML","MP JACK DANIEL'S 1L","MP SMIRNOFF VODKA RED 1L","MP TEQUILA JOSE CUERVO ESPECIAL 750ML","MP JOHNNIE WALKER RED LABEL 1L","MP JOHNNIE WALKER BLACK LABEL 1L","MP ESPIRITO DE MINAS CACHAÇA 970 ML","MP ESPIRITO DE MINAS TRADICIONAL 970ML","MP BALLANTINES 750ML","MP GRANT'S TRIPLE WOOD 750ML","MP APEROL 750 ML","MP BLACK LABEL","MP CACHAÇA AMBURANA","MP CACHAÇA JAMBUCANA","MP CACHAÇA TRADICIONAL","MP COINTREAL","MP CURAÇAU BLUE","MP CURACAU RED","MP ESPUMANTE MOSCATEL","MP LICOR 43","MP RED LABEL","MP RUM BACARDI","MP SAGATIBA PURA 700ML","MP TANQUERAY 750ML","MP TEQUILA PRATA","MP VINHO CABERNET SAUVIGNON","MP VINHO TINTO SUAVE 750ML","MP VODKA ABSOLUT","MP VODKA KETEL ONE BOTANICAS","MP VODKA SMIRNORFF","MP XAROPE DE MORANGO","MP XAROPE GENGIBRE","MP XAROPE LIMÃO SICILIANO","MP YPIOCA OURO 965ML"],
    "NÃO ALCOOLICAS": ["MP AGUA COM GÁS","MP AGUA SEM GÁS","MP PEPSI BLACK","MP TONICA ANTARTICA 355ML","MP SUKITA LATA","MP GUARANA ANTARTICA LATA","MP PEPSI LATA","MP PEPSI BLACK LATA","MP GATORADE MARACUJA","MP REDBULL","MP SUCO DELL VALE 1L","MP GUARANÁ ANTARCTICA 600ML","MP SCHWEPPES GUARANA 600ML","MP SUKITA UVA LATA","MP BARE LATA","MP COCA COLA LATA","MP COCA COLA ZERO LATA","MP FANTA LARANJA LATA","MP FANTA UVA LATA","MP GUARANÁ ANTARTICA ZERO LATA","MP MONSTER MELAN LT 473ML","MP SCHWEPPES CITRUS LATA","MP SCHWEPPES TONICA LATA","MP SODA LIMONADA LATA","MP SPRITE LATA","MP TUCHAUA LATA"],
...
async function carregarMapeamentosInv() {
  ...
  // Adiciona grupos ausentes do código estático na estrutura Centro (recuperação de perdas acidentais)
  Object.entries(_centroBase).forEach(([setor, grupos]) => {
    if (!_todasEstruturas['Centro'][setor]) return;
    Object.entries(grupos).forEach(([grupo, prods]) => {
      if (!_todasEstruturas['Centro'][setor][grupo]) {
        _todasEstruturas['Centro'][setor][grupo] = JSON.parse(JSON.stringify(prods));
        _estruturaMudou = true;
      }
    });
  });
...
// ─────────────────────────────────────────────────────────────────────────────
// TRAVA DO ERRO DE MIL VEZES
//
// Contagem de produto pesado na balanca chega em GRAMAS num campo que e' KG.
// Medido em 28/08/2026 varrendo julho e agosto: 35 produtos, em COZINHA, BAR e
// SALAO. Todo pico e' multiplo de 5 (leitura de balanca) e em 25 deles o mesmo
// numero dividido por mil aparece em outro dia — alho 575 num dia, 0,575 no
// seguinte. Cada erro desses cria uma entrada fantasma no razao num dia e uma
// saida fantasma no dia seguinte. Chegou a R$ 39,6 mil de alface num setor.
//
// A trava so PERGUNTA — nunca bloqueia. Quem conta pode confirmar e seguir, que
// e' o certo quando a compra grande realmente aconteceu.
// ─────────────────────────────────────────────────────────────────────────────
async function _conferirContagemFora(itens, setor, local) {
  try {
    const ids = [...new Set(itens.filter(i => i.produto_id && (i.total || 0) >= 50).map(i => i.produto_id))];
    if (!ids.length) return true;                       // ninguem digitou numero grande

    const desde = new Date(Date.now() - 60 * 864e5).toISOString().slice(0, 10);
    let q = sb.from('est_inventarios').select('id').eq('setor', setor).gte('data', desde);
    if (local) q = q.eq('local', local);
    const { data: invs } = await q;
    if (!invs?.length) return true;                     // sem historico, nao ha o que comparar

    const hist = {};
    const invIds = invs.map(i => i.id);
    for (let i = 0; i < invIds.length; i += 200) {
      const { data: linhas } = await sb.from('est_inventario_itens')
        .select('produto_id,total').in('inventario_id', invIds.slice(i, i + 200)).in('produto_id', ids);
      (linhas || []).forEach(l => {
    ...
    itens.forEach(it => {
      ...
      const h = hist[it.produto_id];
      if (v < 50 || !h || h.length < 3) return;
      h.sort((a, b) => a - b);
      const mediana = h[Math.floor(h.length / 2)];
      if (mediana <= 0 || v < 50 * mediana) return;
      const emKg = v / 1000;
      const pareceGrama = emKg >= mediana / 5 && emKg <= mediana * 5;
      suspeitos.push({ nome: it.nome, v, mediana, emKg, pareceGrama });
    });
    if (!suspeitos.length) return true;

    const fmt = n => n.toLocaleString('pt-BR', { maximumFractionDigits: 3 });
    const linhas = suspeitos.map(s =>
      `• ${s.nome}: você digitou ${fmt(s.v)} — o normal aqui é ${fmt(s.mediana)}` +
      (s.pareceGrama ? `\n   (${fmt(s.v)} gramas seria ${fmt(s.emKg)})` : '')
    ).join('\n');

    return confirm(
      `Confira estes valores antes de salvar:\n\n${linhas}\n\n` +
      `Se a quantidade é essa mesmo, pode continuar.\n` +
      `Salvar assim?`
    );
  } catch (e) {
    console.error('_conferirContagemFora falhou (contagem segue):', e);
    return true;                                        // a trava nunca pode impedir de contar
  }
}
...
async function registrarContagem(rows, { motivo = 'Contagem', responsavel = null, data = null } = {}) {
  ...
  // A estrutura do setor lista dois nomes que caem no mesmo cadastro — "MP
  // POLPA GRAVIOLA" e "MP POLPA GRAVIOLA 1 KG", "MP CONDENSADO LATA" e "MP
  // LEITE CONDENSADO", e mais sete pares. Viram dois campos na tela.
  //
  // Até 31/08/2026 o último vencia, e o campo vazio APAGAVA o que tinha
  // acabado de ser contado: a graviola foi contada 61 e o saldo virou 0,
  // cinco noites seguidas.
  //
  // Fica com o MAIOR, não com a soma. Cheguei a escrever soma e o dado me
  // corrigiu no mesmo dia: em 34 das 40 contagens com as duas linhas
  // preenchidas o time repete o MESMO valor nas duas (leite condensado 1 e 1,
  // 23 vezes; copo 300ml 0,5 e 0,5, 8 vezes) — é o mesmo estoque digitado
  // duas vezes, e somar dobrava. Já contagem repartida de verdade entre as
  // duas linhas não aparece nenhuma vez no histórico.
  //
  // O maior nunca apaga o que foi contado e nunca dobra o que foi repetido.
  // É rede de proteção, não cura: a cura é a linha duplicada sair da
  // estrutura do setor.
  const dedup = {};
  (rows || []).forEach(r => {
    if (!r.produto_id || !r.local) return;
    const k = r.produto_id + '||' + r.local;
    dedup[k] = dedup[k] && (Number(dedup[k].saldo) || 0) >= (Number(r.saldo) || 0)
      ? dedup[k]
      : r;
  });
...
async function confirmarRecebimento() {
  ...
  // Se o pedido ainda está no financeiro como RASCUNHO (integração não aprovada),
...
function caPeriodo(meses) {
  const fim = new Date();
  const ini = new Date(); ini.setMonth(ini.getMonth() - meses);
  document.getElementById('ca-ini').value = ini.toISOString().slice(0, 10);
  document.getElementById('ca-fim').value = fim.toISOString().slice(0, 10);
  renderCurvaABC();
}

async function carregarCurvaABC() {
  if (!cProdutosFT.length) await carregarProdutosFT();
  if (!document.getElementById('ca-ini').value) caPeriodo(3);   // default: 3 meses
  else await renderCurvaABC();
}
...
function _caBadge(c) {
  const cor = { A: ['#dc3545', '#fde8e8'], B: ['#fd7e14', '#fff3e6'], C: ['#6c757d', '#f1f3f5'] }[c] || ['#333', '#eee'];
  return `<span class="badge" style="color:${cor[0]};background:${cor[1]};border:1px solid ${cor[0]}55">${c}</span>`;
}
...
function _renderCurvaKpis(itens, grand, dim) {
  ...
  el.innerHTML = card('A', '#dc3545', '#fde8e8', stat('A'))
              + card('B', '#fd7e14', '#fff3e6', stat('B'))
              + card('C', '#6c757d', '#f1f3f5', stat('C'));
}
...
let _parLinhas = [], _parDia = null, _parPreparos = [], _parDiasVenda = new Set(), _parNoites = 1;

// dia (YYYY-MM-DD) + n dias, sem passar por UTC
function _parSomaDias(dia, n) {
  const d = new Date(dia + 'T12:00:00'); d.setDate(d.getDate() + n);
  return `${d.getFullYear()}-${String(d.getMonth() + 1).padStart(2, '0')}-${String(d.getDate()).padStart(2, '0')}`;
}

// Dia de Manaus de um carimbo do banco. A contagem é física à noite (21h-23h); em UTC
// isso já é o dia seguinte, e o campo `data` das contagens antigas saiu em UTC (ver
// hojeLocal). O dia vem do relógio, igual ao comparativo_diario.py (dia_da_contagem).
function _parDiaManaus(ts) {
  return new Date(ts).toLocaleDateString('en-CA', { timeZone: 'America/Manaus' });
}
...
async function carregarParalelo() {
  ...
  if (!elData.value) elData.value = _parSomaDias(hojeLocal(), -1);   // padrão: ontem, no relógio de Manaus
  const dia = _parDia = elData.value;
  // PERÍODO: quantas noites a conta soma. Uma noite sozinha engana — item contado
  // 0 -> 93 -> 0 parece furo e é ruído de contagem; somar noites cancela esse ruído
  // (leitura de 10/09/2026: a análise que serviu sempre somou 2 a 3 noites).
  const noites = _parNoites = Math.max(1, Number(document.getElementById('par-noites')?.value) || 1);
  tbody.innerHTML = '<tr><td colspan="7" class="text-center text-muted py-4">Carregando…</td></tr>';

  // 14 dias antes do início do período dão folga para achar a contagem anterior de
  // item contado com pouca frequência, sem puxar meses de inventário à toa.
  const dIni = _parSomaDias(dia, -(14 + noites));

  await carregarProdutosFT();
  await _carregarPreparos(dIni, dia);   // antes do resto: não depende de haver contagem

  // um dia a mais de cada lado: o `data` gravado pode estar um dia à frente do dia
  // de Manaus; quem decide o dia é o criado_em (_parDiaManaus)
  const invs = (await _fetchAllPaged('est_inventarios', 'id,setor,data,criado_em,local',
    q => q.gte('data', _parSomaDias(dIni, -1)).lte('data', _parSomaDias(dia, 1))))
    .map(i => ({ ...i, dia: i.criado_em ? _parDiaManaus(i.criado_em) : i.data }))
    .filter(i => i.dia >= dIni && i.dia <= dia);
  ...
  // Entradas vão pelo HORÁRIO, não pelo dia: contam as que chegaram depois da contagem
  // inicial e até a contagem final daquele item. Pelo dia, a emergência recebida às 23:58,
  // depois da contagem das 22h, caía na noite errada.
  const movs = await _fetchAllPaged('est_movimentacoes', 'produto_id,local,data,tipo,quantidade,criado_em',
    q => q.gte('data', dIni).lte('data', _parSomaDias(dia, 1)));
  const ent = {}, venda = {}, vistos = new Set();
  _parDiasVenda = new Set();
  movs.forEach(m => {
    if (_PAR_ENTRADAS.includes(m.tipo)) {
      // CÓPIAS: até 10/09/2026 o mesmo pedido interno entrava até 6x no mesmo minuto
      // (confirmação sem trava, 388 linhas a mais em 9 dias). Mesmo local, produto e
      // quantidade no mesmo minuto conta uma vez só.
      ...
      (ent[`${m.local}|${m.produto_id}`] ||= []).push({ t: Date.parse(m.criado_em), q: Number(m.quantidade) || 0 });
      return;
    }
  ...
  // mediana das contagens positivas de cada (setor, insumo): base para reconhecer a
  // linha deixada EM BRANCO, que a tela grava como 0 — em 9 dias de setembro, 28% do
  // que foi pedido nasceu de linha em branco (ASG 99%)
  const positivos = {};
  Object.entries(cont).forEach(([k, v]) => {
    if (v > 0) { const [s, p] = k.split('|'); (positivos[`${s}|${p}`] ||= []).push(v); }
  });
  const mediana = sp => {
    const vs = (positivos[sp] || []).slice().sort((a, b) => a - b);
    return vs.length >= 3 ? vs[Math.floor(vs.length / 2)] : 0;
  };

  const limite = _parSomaDias(dia, -noites);            // a contagem inicial é a última até aqui
  const linhas = [];
  Object.keys(cont).forEach(k => {
    const [setor, pid, d] = k.split('|');
    if (d !== dia) return;                              // só pares que fecham no dia escolhido
    const antes = dias.filter(x => x <= limite).reverse().find(x => `${setor}|${pid}|${x}` in cont);
    if (!antes) return;                                 // sem contagem anterior não há janela
    const kAntes = `${setor}|${pid}|${antes}`;
    const janela = dias.filter(x => x > antes && x <= dia);
    const t0 = Date.parse(quando[kAntes]), t1 = Date.parse(quando[k]);
    const somaEnt = (ent[`${setor}|${pid}`] || []).reduce((s, e) => s + (e.t > t0 && e.t <= t1 ? e.q : 0), 0);
    const c0 = cont[kAntes], c1 = cont[k];
    const real  = c0 + somaEnt - c1;
    ...
    linhas.push({
      ...
      unSuspeita: fator !== 1,
      // zero numa ponta de item que costuma ter estoque = linha deixada em branco, não
      // consumo (as duas pontas em zero também): aparece na lista, fica fora do placar
      branco: (c0 === 0 && c1 === 0) || ((c0 === 0 || c1 === 0) && mediana(`${setor}|${pid}`) > 0),
    });
  });
...
// RESUMO POR SETOR — a mesma conta do placar, quebrada por setor, para ler o período
// sem somar linha por linha. Só linhas limpas; o que ficou de fora aparece contado.
function _pintarResumoParalelo(limpas) {
  const el = document.getElementById('par-resumo');
  if (!el) return;
  if (!limpas.length) { el.innerHTML = ''; return; }
  const por = {};
  limpas.forEach(l => {
    const r = por[l.setor] ||= { n: 0, batem: 0, erro: 0, consumo: 0, venda: 0 };
    r.n++;
    r.erro += Math.abs(l.valor);
    r.consumo += Math.abs(l.real * l.custo);
    r.venda += l.modelo * l.custo;
    if (l.razao !== null && l.razao >= 0.75 && l.razao <= 1.25) r.batem++;
  });
  const fora = s => _parLinhas.filter(l => l.setor === s && (l.branco || l.unSuspeita)).length;
  const corpo = Object.entries(por).sort((a, b) => b[1].consumo - a[1].consumo).map(([s, r]) => {
    const p = r.consumo > 0 ? (r.erro / r.consumo) * 100 : null;
    return `<tr>
      <td><a href="#" onclick="document.getElementById('par-setor').value='${esc(s)}';_pintarParalelo();return false">${esc(s)}</a></td>
      <td class="text-end">${r.n}</td>
      <td class="text-end">${brl(r.consumo)}</td>
      <td class="text-end">${brl(r.venda)}</td>
      <td class="text-end">${brl(r.erro)}</td>
      <td class="text-end fw-semibold" style="color:${p !== null && p <= 10 ? '#16a34a' : '#b45309'}">${p === null ? '—' : pct(p)}</td>
      <td class="text-end">${r.batem} de ${r.n}</td>
      <td class="text-end text-muted">${fora(s) || '—'}</td>
    </tr>`;
  }).join('');
  el.innerHTML = `<div class="card-grafico p-0 overflow-hidden"><div class="table-responsive">
    <table class="table table-sm align-middle mb-0">
      <thead class="table-light"><tr>
        <th>Setor</th><th class="text-end">Itens limpos</th><th class="text-end">Contagem diz</th>
        <th class="text-end">Venda diz</th><th class="text-end">Erro bruto</th>
        <th class="text-end">Erro sobre o contado</th><th class="text-end">Batem (±25%)</th>
        <th class="text-end" title="Linha em branco ou unidade não curada: aparecem na lista, fora do placar">Fora da conta</th>
      </tr></thead>
      <tbody>${corpo}</tbody>
    </table></div></div>`;
}
...
function _pintarParalelo() {
  ...
  // PLACAR só com as linhas LIMPAS: linha em branco e unidade não curada aparecem na
  // lista (com selo), mas não entram no erro — senão o placar mede o preenchimento da
  // contagem, não a ficha. Mesma separação da leitura de 10/09 (limpo / zero / unidade).
  const limpas = todas.filter(l => !l.branco && !l.unSuspeita);
  const brancos = todas.filter(l => l.branco).length;
  const batem = limpas.filter(l => l.razao !== null && l.razao >= 0.75 && l.razao <= 1.25).length;
  const bruto = limpas.reduce((s, l) => s + Math.abs(l.valor), 0);
  ...
  const consumo = limpas.reduce((s, l) => s + Math.abs(l.real * l.custo), 0);
  ...
  kpis.innerHTML = !todas.length ? ''
    : semVenda
      ...
      : _relKpiChip('Insumos limpos', `${limpas.length} de ${todas.length}`, '#0d6efd')
        + _relKpiChip('Batem (±25%)', `${batem} de ${limpas.length}`, limpas.length && batem / limpas.length >= 0.7 ? '#16a34a' : '#b45309')
        ...
        + (brancos ? _relKpiChip('Linha em branco (fora)', String(brancos), '#6b7280') : '')
        + (suspeitas ? _relKpiChip('Unidade não curada (fora)', String(suspeitas), '#6b7280') : '');
  if (cont) cont.textContent = todas.length ? `${_parNoites} noite(s) até ${_dataBR(_parDia)} · ${todas.length} insumo(s) com movimento` : '';
  _pintarResumoParalelo(todas.length && !semVenda ? limpas : []);
  ...
  tbody.innerHTML = linhas.map(l => {
    ...
    return `<tr>
      <td>${esc(l.insumo)}${l.unSuspeita ? ' <span class="badge bg-warning text-dark" title="Unidade de compra diferente da de uso: a diferença pode ser só a unidade da contagem, não furo de estoque.">un?</span>' : ''}${l.branco ? ' <span class="badge bg-secondary" title="Uma das contagens (início ou fim) está em zero para um item que costuma ter estoque: provavelmente a linha foi deixada em branco. Fica fora do placar.">em branco?</span>' : ''}</td>
      ...
      <td class="text-end" title="Contou ${_parNum(l.c0)} em ${esc(_dataBR(l.de))} + entrou ${_parNum(l.entrou)} − contou ${_parNum(l.c1)} em ${esc(_dataBR(_parDia))}">${_parNum(l.real)} <span class="text-muted small">${esc(l.unidade)}</span></td>
